...
export const ProjectionView: React.FC<ProjectionProps> = ({ workshopId, isReadOnly = false }) => {
  ...
  const renderScreenContent = () => {
    switch (workshop.currentProjectionScreen) {
      ...
      case 'P15':
        return (
          <div style={{ maxWidth: '600px', width: '100%', margin: '0 auto', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <h1 style={{ fontSize: '3rem', color: 'var(--accent)', marginBottom: '1.25rem', fontWeight: 800 }}>
              學員手機端畫面範例
            </h1>
            <p style={{ fontSize: '1.25rem', opacity: 0.6, marginBottom: '2rem' }}>
              請各位學員用手機進行 DoD 驗收與交付，每關都需完成以下步驟：
            </p>
